...
## Commits
Krenut ćemo sa dodavanjem plugina kroz NPM i sa podešavanjem. Nakon toga stvaramo par novih komponenti i page te na kraju radimo "programsko (dinamično) stvaranje stranica". Za to se koristi `gatsby-node.js`. O tom po tom :)
...
Ako se stranica pokrene bez uozorenja onda nije potrebno. Meni je došao eksplicitno upozorenje da podignem verziju gatsbya (tkz. _bump_)
...
Ne želimo stavit ključ tako da ga svi vide, ali ako ga ne stavimo ne možemo koristiti plugin. Riješenje je process enviorment objekt.
#### `.env` file
Prvo stvaramo novi file: `.env` u rootu projekta. Taj file onda popunimo sa ključevima:
```toml
SPACE_ID=ayujigrtz540
ACCESS_KEY=zgu4Py2RSPno6PLBhZkPtn38UFD2RL.CQgKGbbLmsKE
```
> Ključ koji vidite nije pravi i neće radit
...
Ako napravimo `gatsby-develop` možemo vidjet novi source u `graphql` exploreru:

<p align="center">
<img src="./readmeRess/gsc1.png">
</p>

Time završavamo sa konfiguracijom (za sad).
...
Za sada to je samo layout, ako što se vidi.
...
Query koji vidimo na vrhu stranice dohvaća blogove, a ova komponenta ih onda mapira i prikazuje u listi. Primjetite `<Link />` koji omotava svaki element liste. Moguće je kliknuti na svaki od blogova i otovriti ga na novoj stranici.
...
Prvo, da bi ovo moglo radit mora postojiati `./src/layouts/blog.js`. Njega ćemo stvoriti uskoro.
...
Sad kad znamo što se događa ovdje (bar djelimočno), napravimo blog:
...
Bitno je primjetit `pageContext` koji komponenta prima. To je isti onaj context koji šalje `createPage` funkcija!  
Unutar njega se nalazi sam objekt koji sadrži blog i dva objekta:
- next
- prev

Koji sadrže sljedeći i predhodni blog.
