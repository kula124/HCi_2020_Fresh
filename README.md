...
Ulazimo u zadnju fazu projekta.
...
Za početak riješimo `ContactSeparator`. Očito, moramo promijeniti to ime da se može koristiti općenito. Mijenjanjem imena moramo promijeniti i import. Također, dodajemo mu prop za prikaz proizvoljnog teksta. Promijenit ćemo mu ime u `SeparatorBar` i dodat ćemo mu `text` prop. Zatim prelazimo na galerije slika.

- [**Commit 1: ContactSeparator refactor**](#c1)
...
``` 
Imamo ga. Sad trebamo **special case** kad je index 3. Radimo provjeru i ako je index jednak 3 uzimamo 4 * 4, ako ne onda ostaje kako je. Ako vas to vuče na ternarni operator, čestitam! :)
```jsx
import React from 'react'
