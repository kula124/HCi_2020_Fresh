...
Commitovi će sadržavati kod za doradu starih komponenti i dodavanje novih. Inače, te dvije radnje se rade u dva brancha umjesto u jednom. To će biti naznačeno. Budući da su ovo vježbe, ostavljeno je ovako.
  - [**Commit 1: creating accutal Combobox**](#c1)
    ...
    - Dodajemo novi kod za `Combobox`
    - Dodajemo `<ComboboxItem />`
    - CSS
  - [**Commit 2: Creating Combobox: Internal logic**](#c2)
    - Definitamo state varijable
    - Uvjetujemo render
    - Dodajemo `onClick` funkcije
    - Dodajemo `selected` prop
  - [**Commit 3: rework: adding FontAwsome**](#c3)
    - Dodajemo *FontAwsome*
    - Dodajemo ikone u `ContactBar`
  - [**Commit 4: rework: adding Combobox picker**](#c4)
    - Dodajemo `react-flags-select` 
    - Importamo ga umjesto `<Combobox />`
  - [**Commit 5: adding gatsby's background image**](#c5)
...
## Commit 1: Creating Combobox: Look and style <a name="c1"></a>
- Dodajemo novu konstantu
- Dodajemo novi kod za `Combobox`
- Dodajemo `<ComboboxItem />`
- CSS
...
Za početak dodajmo u `src/constants` niz koji drži ponuđene jezike i slike koje idu uz njih:
```jsx
...
Sad imamo nacrt za `ComboboxItem`. Mora primati ta tri polja kroz svoj `props`. Napravimo sad `ComboboxItem` u `components/Combobox` folderu:
<p align="center">
...
```sh
$ git add .
$ git commit -m "Creating Combobox: Look and style"
```

## Commit 2: Creating Combobox: Internal logic<a name="c2"></a>

- Definitamo state varijable
- Dodajemo `selected` u `ComboboxItem`
- Dodajemo `onClick` funkcije
- Dodajemo `selected` prop

### Logika i state <a name="props"></a>
Vidimo da je `Combobox` sad uvjek otvoren. Želimo imati varijablu koja će pamtiti stanje komponente, tj. je li otvorena ili zatvorena.  
Također, moramo znati koji je jezik trenutno odabran. Jezik koji je odabran ima poseban CSS stil i mora biti prikazan u komponenti dok je zatvorena.

Idemo napraviti logiku za `Combobox` koristeći React state koji smo upoznali ranije. Očito je da taj state ima dvije varijable:
  - Trenutno odabran jezik
  - Je li Combobox otvoren

Vrijeme je da uvedemo `useState` i definiramo dvije state varijable:
```jsx
import React, {useState} from 'react'
...
export default Combobox
```
Dodali smo i `selectedItem.language` i prazan objekt u `useState`. Da nismo, `selectedItem.language` bi bacio error jer je selectedItem undefined na početku.

Sad u ovisnosti o `open` trebamo pikazati `<ul>` sadržaj. Kako?  
Dva načina:
- Ternarni operator i `null`
- `&&` operator
...
```jsx
const ComboboxItem = ({language, name, flag, setSelected}) => (
    <li key={name}
...
```
Sad vidimo da radi kako treba, ali na samom početku `Combobox` je prazan. Riješimo i to sa `useState` i `.find()`. Također, stavimo `open` na false na početku:
...
Još dvije stvari. Definirajmo klasu (i prop) `selected` u `ComboboxItem`. Treba nam da bismo iskoristili `selectedItem` state. Nije teško, jedna usporedba i jedan ternarni operator :)
...
```
I dodatak u `className` unutar `ComboboxItem` i `selected` propa:
```jsx
...
```sh
$ git add .
$ git commit -m "Creating Combobox: Internal logic"
```
 
[Sadržaj git commitova](#toc)
 
## Commit 3: rework: adding FontAwsome<a name="c3"></a>
  - Dodajemo *FontAwsome*
  - Dodajemo ikone u `ContactBar`
...
## Commit 4: rework: adding Combobox picker<a name="c4"></a>
- Dodajemo `react-flags-select` 
- Importamo ga umjesto `<Combobox />`
...
## Commit 5: adding gatsby's background image<a name="4">
- Dodajemo `gatsby-background-image` paket
- Skidamo sliku u `src/images/titleImage.jpg`
- Koristimo `gatsby-background-image` umjesto CSS-a
...
## Commit 6: contact page init <a name="c6"></a>
  - Dodajemo page u `pages/contact.js`
  - Dodajemo navigacijski link
  - Dodajemo `<layout/>`
  - Dodajemo `<ContactSeparator />`
...
## Commit 7: contact page ContactForm <a name="c7"></a>
- Stvaramo `ContactForm` modul
- Dodajemo `ContactForm` u `pages`
